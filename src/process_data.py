...
from fit_regressor import (
    build_linear_model,
    build_randforest_model,
    evaluate_model,
)
...
def print_top_n_price(df: gpd.GeoDataFrame, column_name: str, top_n: int = 3):
    """
    ...
    for i in range(top_n):
        print(
            f"\t{df[column_name].nlargest(top_n).index.to_numpy()[i]}: "
            f"£ {np.round(df[column_name].nlargest(top_n).values[i], 0)} per night."
        )
...
def main():
    ...
    if len(sys.argv) == 5:
        ...
        linear_train_r2, linear_train_rmse = evaluate_model(
            model_linear, X_train, y_train
        )

        randfo_r2, randfo_rmse = evaluate_model(model_randfo, X_test, y_test)
        randfo_train_r2, randfo_train_rmse = evaluate_model(
            model_randfo, X_train, y_train
        )
        ...
        print(
            "Please provide the filepaths of the listings, listings summary, and neighbourhoods "
